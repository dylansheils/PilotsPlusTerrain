...
class Region:
    ...
    def __init__(self, region_name):
        ...
        self.best_resolution = 16
        sample_points = self.load_full(16)
    ...
    def load_region(self, new_location):
        global rtree_index, epsilon

        print("Examining Region...")
        ...
        if(change):
            # Update the KD-tree with new data
            self.update_kdtree()

    def update_kdtree(self):
        global kdtree
        """Update the KD-tree with new points."""
        if len(self.kdtree_data) > 0:
            kdtree = KDTree(self.kdtree_data)
    ...
    def load_into_mesh(self, sample_points, resolution):
        ...
        new_points = np.column_stack((lats, lons, elevations + 50))  # Create an array of new points
    ...
    def detect_intersection(self, start, end):
        global kdtree
        start_lat, start_lon, start_elev = start
        end_lat, end_lon, end_elev = end

        # Check for intersecting points using KD-tree
        if kdtree:
            query_point = np.array([[(start_lat + end_lat) / 2, (start_lon + end_lon) / 2, (start_elev + end_elev) / 2]])
            # Define distance threshold for elevation
            elev_threshold = 500.0

            # Query the KD-tree with distance thresholds
            dist, idx = kdtree.query(query_point, k=50, distance_upper_bound=50.0)

            # Check if any of the conditions are met for intersection
            for i in range(len(dist)):
                if (
                    np.any(idx[i] < len(self.kdtree_data)) and
                    np.any(np.linalg.norm(self.kdtree_data[idx[i]][2] - query_point[0][2]) <= elev_threshold)
                ):
                    return True
        return False
...
def main():
    ...
    n43w74region = Region(region)
    interpolated = n43w74region.interpolate_resolution(16, (0, 0, 0))
    print("Interpolated Latitude:", interpolated[0], "Interpolated Longitude:", interpolated[1])
    interpolated = n43w74region.interpolate_resolution(16, (1, 1, 0))
    print("Interpolated Latitude:", interpolated[0], "Interpolated Longitude:", interpolated[1])

    # Define the start and end points
    end_point = (-73.99, 42.99, 8000)
    start_point = (-73.00279259217973, 42.0027925922772, 0)

    # Number of points to generate
    num_points = 100  # You can adjust this number as needed

    # Generate linearly spaced points between start and end
    latitudes = np.linspace(start_point[0], end_point[0], num_points)
    longitudes = np.linspace(start_point[1], end_point[1], num_points)

    # Combine latitudes and longitudes
    generated_points = list(zip(latitudes, longitudes, np.linspace(8000, 0, num_points)))
    prior = start_point
    #n43w74region.load_full(4)
    for point in generated_points:
        n43w74region.load_region(point)
        print(point, n43w74region.detect_intersection(point, prior))
        prior = point
