...
double inscribed_c1(Seg path, double radius)
{
    ...
    return (fabs(inscribed_angle2+inscribed_angle1));
}
...
//detect inscribed angle in c1
	double inscribed_angle= inscribed_c1(path, radius);

	int i;
	double step = inscribed_angle/(path.lenc1-1);	
	for(i=0;i<path.lenc1;i++)
	{
...
//detect inscribed angle in c2
	double inscribed_angle= inscribed_c2(path, radius);

	int i;
	double step = inscribed_angle/(path.lenc2-1);
	for(i=0;i<path.lenc2;i++)
	{
		collection.thetas[i]= theta_first + (i * (step * sign));
	}
...
Curve wind_curveA(Seg path,double wind_heading,double wind_velocity,double omega, double radius, double start_altitude, double initial_x, double initial_y, int angle, double baseline_g, double airspeed)
{
	...
	Thetas theta_pool=generate_thetasA(path, initial_centre, radius);	//pool od thetas to use	
...
Curve wind_curveB(Seg path, double wind_heading, double wind_velocity, double omega, double radius, Curve augmented_SLS, int angle, double baseline_g, double airspeed, double rnwy_heading)
{
	...
	Thetas theta_pool=generate_thetasB(path, original_c2_centre, radius);	//pool od thetas to use	
...
// Function to create and populate a 4D array
double** create_4d_array(Curve curve) {
    int i, idx = 0;
    int num_points = curve.len_curve * 2; // Assuming curve A and curve B are always present

    // Include points from the spiral if it exists
    if (curve.spiral) {
        num_points += curve.len_curve;
    }

    // Include points from the extended if it exists
    if (curve.extended) {
        num_points += curve.len_curve;
    }
    ...
    // Populate the array with points from curve A and B
    for (i = 0; i < curve.len_curve; i++) {
    	array[idx] = (double*)malloc(4*sizeof(double));
        array[idx][0] = curve.points[i][0];
        array[idx][1] = curve.points[i][1];
        array[idx][2] = curve.points[i][2];
        array[idx][3] = curve.points[i][4] * 364173;
        idx++;
    }

    // Populate the array with the starting point of SLS
    array[idx] = (double*)malloc(4*sizeof(double));
    array[idx][0] = curve.points[0][0];
    array[idx][1] = curve.points[0][1];
    array[idx][2] = curve.points[0][2];
    array[idx][3] = curve.points[0][4] * 364173;
    idx++;

    // Populate the array with the ending point of B
    array[idx] = (double*)malloc(4*sizeof(double));
    array[idx][0] = curve.points[curve.len_curve - 1][0];
    array[idx][1] = curve.points[curve.len_curve - 1][1];
    array[idx][2] = curve.points[curve.len_curve - 1][2];
    array[idx][3] = curve.points[curve.len_curve - 1][4] * 364173;
    idx++;

    // Populate the array with points from the spiral if it exists
    if (curve.spiral) {
        for (i = 0; i < curve.len_curve; i++) {
        	array[idx] = (double*)malloc(4*sizeof(double));
            array[idx][0] = curve.points[i][0];
    ...
    // Populate the array with points from the extended if it exists
    if (curve.extended) {
        for (i = 0; i < curve.len_curve; i++) {
        	array[idx] = (double*)malloc(4*sizeof(double));
            array[idx][0] = curve.points[i][0];
...
double** merge_curves(Curve *curves, int num_curves) {
    ...
    for (int i = 0; i < num_curves; ++i) {
        total_len += curves[i].len_curve;
    }

    // Allocate memory for the merged array
    double** merged_array = (double**)malloc((total_len+1) * sizeof(double*));
    ...
    for (int i = 0; i < num_curves - 1; ++i) {
        ...
        for (int j = 0; j < curves[i].len_curve; ++j) {
        	index = idx++;
        	merged_array[index] = (double*)malloc(4*sizeof(double));
            merged_array[index][0] = temp_array[j][0];
